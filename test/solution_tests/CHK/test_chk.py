...
from solutions.CHK.offers import MultiPriceOffer, GroupDiscountOffer
...
class TestChk():
    ...
    def test_singleItemOffer(self):
        offerContents = {'A': 1}
        offerPrice = 10
        offer = MultiPriceOffer(offerContents, offerPrice, TEST_PRICES)
    ...
    def test_multipleOffers(self):
        of1 = MultiPriceOffer({'A': 1}, 10, TEST_PRICES)
        of2 = MultiPriceOffer({'B': 1}, 10, TEST_PRICES)
    ...
    def test_multipleCompetingOffers(self):
        of1 = MultiPriceOffer({'A': 1}, 20, TEST_PRICES)
        of2 = MultiPriceOffer({'A': 4}, 60, TEST_PRICES)
    ...
    def test_two_e_one_b_free(self):
        offer = MultiPriceOffer({'E': 2, 'B': 1}, chk.getTotalPrice({'E': 2}), TEST_PRICES)
    ...
    def test_two_f_get_one_f_free(self):
        offer = MultiPriceOffer({'F': 3}, chk.getTotalPrice({'F': 2}), TEST_PRICES)
    ...
    def test_singleItemGroupDiscountOffer(self):
        offer = GroupDiscountOffer(['A'], 1, 10, TEST_PRICES)
        ...
        offer = GroupDiscountOffer(['A', 'B'], 1, 10, TEST_PRICES)
        assert chk.checkout('A', TEST_PRICES, [offer]) == 10
        assert chk.checkout('B', TEST_PRICES, [offer]) == 10
        offer = GroupDiscountOffer(['B', 'A'], 1, 10, TEST_PRICES)
        assert chk.checkout('A', TEST_PRICES, [offer]) == 10
        assert chk.checkout('B', TEST_PRICES, [offer]) == 10

    def test_multiItemGroupDiscountOffer(self):
        offer = GroupDiscountOffer(['A', 'B', 'C'], 2, 20, TEST_PRICES)
        assert chk.checkout('AA', TEST_PRICES, [offer]) == 20
        ...
        assert chk.checkout('ABC', TEST_PRICES, [offer]) == 20 + min([TEST_PRICES[c] for c in "ABC"])
        assert chk.checkout('CBA', TEST_PRICES, [offer]) == 20 + min([TEST_PRICES[c] for c in "ABC"])

    def test_groupDiscountOfferMultipleTimes(self):
        offer = GroupDiscountOffer(['A', 'B'], 2, 10, TEST_PRICES)
    ...
    def test_multipleGroupDiscountOffers(self):
        of1 = GroupDiscountOffer(['A', 'B'], 2, 10, TEST_PRICES)
        of2 = GroupDiscountOffer(['C', 'D'], 2, 10, TEST_PRICES)
    ...
    def test_groupDiscountAndMultiBuy(self):
        ...
        of2 = MultiPriceOffer({'A': 1, 'B': 1, 'C': 1}, 30, TEST_PRICES)
        assert chk.checkout('ABC', TEST_PRICES, [of1, of2]) == 20
        of1 = GroupDiscountOffer(['A', 'B', 'C'], 3, 40, TEST_PRICES)
        of2 = MultiPriceOffer({'A': 1, 'B': 1, 'C': 1}, 30, TEST_PRICES)
